...
function InfoListPlace() {
  ...
  const { pointPlace } = useSelector((state: rootState) => state.scheduleCreate);
  const timer = useMemo(() => {
  ...
  const deletePlace = (id: number) => {
    ...
    dispatch(scheduleActions.setPlaceList({ place: newPlaceList }));
  };

  const deletePlaceAll = () => {
    dispatch(scheduleActions.setPlaceList({ place: [] }));
  };

  const deletePointPlace = (id: number) => {
    dispatch(scheduleActions.deletePointPlaceList({ position: id, value: null }));
  };
  ...
  const onChangeAccount = (index: number, e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    dispatch(scheduleActions.setPlaceTime({ index: index, time: timeToString === "0:0" ? "0:1" : timeToString }));
  };
